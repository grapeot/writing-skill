...
A hard requirement for an external deliverable, reducing cognitive load rather than decorating. A short article (<2,000 words) needs ≥1 image; a long one needs ≥2-3. Every image entering the final Markdown must be generated or redrawn by the image generation/redraw tool available in the current workspace, compressed to JPG/WebP with a long edge of about 1024px and each image under 200KB, with a relative path and alt text written as a full judgment sentence; for quantitative visuals, first draw the data accurately with a deterministic tool, then hand it to the image model to redraw. An infographic stays faithful to the concept's real topology, and a parallel relationship is not drawn as a linear pipeline for the sake of tidiness.

Visual style defers to the publishing channel: if the workspace's publish skill declares a site visual language spec, every image must follow that spec's composition grammar, semantic palette, rendering tier, and text discipline, tracking its updates; on conflict, the site spec wins and no details are duplicated here. If the workspace has no site spec, fall back to this pinned summary (pinned from the yage.ai/share site visual language spec, 2026-08-20):

- Composition, choose one: contrast panels (A vs B) / transmission chain (causal flow, 3-5 nodes) / layered panorama (hierarchy); one image carries exactly one judgment.
- Semantic four colors: cream base `#F6F2E7`; structure `#6B7FE8` (line-art `#8298FF`); money/value `#E8A33D` (sparingly); negative signal `#C6574A` (line-art `#C26B5A`, at most one element); lines/text `#2E3442` (line-art `#3A4150`).
- Rendering, choose one tier: pixel tier is the default (flat solid fills, chunky pixels, 8-16 colors, no gradients, no anti-aliasing) for mechanism metaphors and concept contrasts; line-art tier for data/protocol/policy visuals that depend on precise annotation; never mix tiers within one article.
- Minimal in-image text: 2-6 character short labels or bare numbers; full sentences never enter the image.
